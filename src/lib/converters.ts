...
export async function pdfToImage(file: File): Promise<ConvertedFile[]> {
...
export async function pdfRotate(file: File, angle = 90): Promise<ConvertedFile> {
  ...
  return postFormData(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfRotate}`,
    fd
  );
}
...
export async function pdfWatermark(
  ...
  // =====================================================
  // NEW: workflow document identity
  // =====================================================
  ...
  if (account) {
    try {
      ...
      console.warn("Watermark token fetch failed:", err);
    }
  }

  // =====================================================
  // HEADERS (UNIFIED SYSTEM)
  // =====================================================
  ...
  const res = await fetch(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfWatermark}`,
    {
      method: "POST",
      headers,
      body: fd,
...
export async function pdfStamp(
  ...
  return postFormData(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfStamp}`,
    fd
  );
}
...
export async function pdfSign(
  ...
  return postFormData(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfSign}`,
    fd
  );
}
...
export async function pdfExtractText(file: File): Promise<any> {
  ...
  const res = await fetch(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfEditExtract}`,
    { method: 'POST', body: fd }
  );
...
export async function pdfEdit(file: File, updates: any[]): Promise<ConvertedFile> {
  ...
  const res = await fetch(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfEditUpdate}`,
    { method: 'POST', body: fd }
  );
...
export async function wordToExcel(file: File): Promise<ConvertedFile> {
  ...
  return postFormData(
    `${getBaseUrl()}${API_CONFIG.endpoints.wordToExcel}`,
    fd
  );
}
...
export async function pdfToPpt(file: File): Promise<ConvertedFile> {
  ...
  return postFormData(
    `${getBaseUrl()}${API_CONFIG.endpoints.pdfToPpt}`,
    fd
  );
}